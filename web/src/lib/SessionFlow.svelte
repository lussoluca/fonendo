...
<div class="flow-head">
  <h2>session flow</h2>
  {#if meta?.name}
    <span class="session-name" title="name set with /rename">{meta.name}</span>
  {/if}
  {#if meta?.title}
    <span class="session-title" title="title generated by Claude Code">{meta.title}</span>
  {/if}
  <span class="sub">
